...
trap_init(void)
{
	...
	// constantes T_DIVIDE, T_DEBUG, etc definidas en inc/trap.h
	...
	// SETGATE(idt[T_COPROC],0,GD_KT,trap_9,0);//
	...
	// SETGATE(idt[T_RES],0,GD_KT,trap_15,0);//
...
trap_init_percpu(void)
{
	...
	// En lugar de usar la variable global ts, se use el campo cpu_ts del
	// arreglo global cpus Para ello basta con eliminar la variable global
	// ts, y definirla así dentro de la función:
	int id = cpunum();
	struct CpuInfo *cpu = &cpus[id];
	struct Taskstate *ts = &cpu->cpu_ts;

	// el campo ts->ts_ss0 seguirá apuntando a GD_KD;
	// pero ts->ts_esp0 se deberá inicializar, como en tareas anteriores,
	// de manera dinámica según el valor de cpunum().
	ts->ts_esp0 = KSTACKTOP - id * (KSTKSIZE + KSTKGAP);
	ts->ts_ss0 = GD_KD;
	ts->ts_iomb = sizeof(struct Taskstate);

	// Calcular segmento e índice para cada core adicional
...
trap_dispatch(struct Trapframe *tf)
{
	...
	// LAB 3: Your code here.
	// decide a qué otra función de C invocar según el valor de tf->tf_trapno:
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 4: Your code here.
	// Destroy the environment that caused the fault if 'env_pgfault_upcall' is null
	if (!curenv->env_pgfault_upcall) {
		cprintf("env_pgfault_upcall is null.\n");
	...
	user_mem_assert(
	        curenv, (void *) utf, sizeof(struct UTrapframe), PTE_W | PTE_U);
