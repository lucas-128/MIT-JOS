...
sys_exofork(void)
{
	...
	struct Env *env;
	int env_alloc_ret = env_alloc(&env, curenv->env_id);
	// Returns < 0 on error.
	if (env_alloc_ret < 0)
	...
	// Registro EAX guarda valor de retorno de funcion.
...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	// que el Trapframe recibido como parámetro
	// apunta a memoria de usuario válida
	int r;
	...
	// Que el proceso no va a salir del ring 3
	// (esto es, que el CPL de todos sus segmentos es 3)
	env->env_tf.tf_ds |= 3;
	env->env_tf.tf_es |= 3;
	env->env_tf.tf_ss |= 3;
	env->env_tf.tf_cs |= 3;
...
sys_env_set_pgfault_upcall(envid_t envid, void *func)
{
	...
	// struct Env's 'env_pgfault_upcall' field.  When 'envid' causes a page
	// fault, the kernel will push a fault record onto the exception stack,
	// then branch to 'func'.
...
sys_page_map(envid_t srcenvid, void *srcva, envid_t dstenvid, void *dstva, int perm)
{
	...
	// -E_INVAL if dstva >= UTOP or dstva is not page-aligned.
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((int) srcva < UTOP && ((perm & PTE_P) == 0 || (perm & PTE_U) == 0 ||
	                           ((PTE_SYSCALL & perm) != perm)))
		return -E_INVAL;
	...
	} else {  // page not transferred.
		// env_ipc_perm is set to 'perm' if a page was transferred, 0 otherwise.
		perm = 0;
	}

	// The send succeeds, and the target's ipc fields are updated as
	// follows: env_ipc_recving is set to 0 to block future sends;
...
sys_ipc_recv(void *dstva)
{
	...
	//  marcar el proceso como ENV_NOT_RUNNABLE
	curenv->env_status = ENV_NOT_RUNNABLE;

	//  poniendo a true el flag thisenv->env_ipc_recving
	curenv->env_ipc_recving = true;

	// 'dstva' is the virtual address at which the sent page should be mapped.
	curenv->env_ipc_dstva = dstva;

	// set registro eax en 0 como valor de retorno
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	/*
	 * Implementar en syscall() soporte para cada tipo de syscall definido
	 *en inc/syscall.h. Se debe devolver -E_INVAL para números de syscall
	 *desconocidos. Nota: solo hace falta despachar, desde syscall(), cada
	 *tipo a las funciones estáticas ya implementadas: SYS_cputs a
	 *sys_cputs(), SYS_getenvid a sys_getenvid(), etc.
	 * */
