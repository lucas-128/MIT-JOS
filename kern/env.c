...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for (int i = 0; i < npages; i++) {
		...
		page_insert(e->env_pgdir, page, (void *) start, PTE_W | PTE_U);
...
load_icode(struct Env *e, uint8_t *binary)
{
	...
	struct Proghdr *programhdr =
	        (struct Proghdr *) ((uint8_t *) binary + elf->e_phoff);
	struct Proghdr *end = programhdr + elf->e_phnum;

	for (; programhdr < end; programhdr++) {
		if (programhdr->p_type != ELF_PROG_LOAD) {
			continue;
		}
		region_alloc(e, (void *) programhdr->p_va, programhdr->p_memsz);
		// copia filesz bytes desde binary + offset a va
		memcpy((void *) programhdr->p_va,
		       binary + programhdr->p_offset,
		       programhdr->p_filesz);
		// escribe a 0 el resto de bytes desde va + filesz hasta va + memsz
		memset((void *) programhdr->p_va + programhdr->p_filesz,
		       0,
...
env_create(uint8_t *binary, enum EnvType type)
{
	...
	// And sets its env_type.
	e->env_type = type;

	// Carga el binary usando load_icode
	load_icode(e, binary);
